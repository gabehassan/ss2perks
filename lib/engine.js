...
export function computeStats(build, perksMap) {
  ...
  // Boss arrival for stats panel
  result._BossArrival = 780 - (result.BossSpawnSecondsEarly || 0);
...
export function formatStat(value, format, base) {
  switch (format) {
    ...
    case 'intChange': {
      // Show as change from base: e.g. 55 with base 100 => "-45"
      const diff = Math.round(value - (base || 0));
      if (diff === 0) return Math.round(value).toString();
      return (diff > 0 ? '+' : '') + diff;
    }
    case '1f':
    ...
    case 'pct100':
      // Show raw value * 100 as percent: 0.05 => "5%", 0.13 => "13%"
      return Math.round(value * 100) + '%';
    case 'multPct': {
      // Multiplier shown as +X% change from base: 1.2 with base 1 => "+20%"
      const pctChg = Math.round(((value / (base || 1)) - 1) * 100);
      if (pctChg === 0) return 'Base';
      return (pctChg > 0 ? '+' : '') + pctChg + '%';
    }
    case 'pctChange': {
      // Combined multiplier: 1.39 => "+39%"
      const pct = Math.round((value - 1) * 100);
      if (pct === 0) return '+0%';
      return (pct > 0 ? '+' : '') + pct + '%';
    }
    case 'pctAdd': {
      // Add-based percentage: 3.0 => "+300%"
    ...
    case 'time': {
      // Seconds => MM:SS
      const totalSec = Math.max(0, Math.round(value));
...
export function getStatDirection(value, format, base, inverted) {
  ...
  const isIncreased = value > baseRef;
